...
def visualize(render_view, base_color, normal, metallic, roughness, mask, base_color_pred, normal_pred, metallic_pred, roughness_pred, mean, std):
    ...
    base_color = np.concatenate([render_view[..., ::-1], base_color_pred[..., ::-1], base_color[..., ::-1]], axis=1)
    normal = np.concatenate([render_view[..., ::-1], normal_pred[..., ::-1], normal[..., ::-1]], axis=1)
    metallic = np.concatenate([render_view[..., ::-1], cv2.cvtColor(metallic_pred, cv2.COLOR_GRAY2BGR), metallic[..., ::-1]], axis=1)
    roughness = np.concatenate([render_view[..., ::-1], cv2.cvtColor(roughness_pred, cv2.COLOR_GRAY2BGR), roughness[..., ::-1]], axis=1)
    
    img = np.concatenate([base_color, normal, metallic, roughness], axis=0)
...
def train(vis=False):
    ...
    for epoch in tqdm.tqdm(range(num_epochs)):
        for i, (render_view, base_color, normal, metallic, roughness, mask, recon_view) in enumerate(train_loader):
            ...
            # Visualization
            if vis:
                visualize(render_view, base_color, normal, metallic, roughness,
                        mask, base_color_pred, normal_pred, metallic_pred, roughness_pred,
                        mean, std)
